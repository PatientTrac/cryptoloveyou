;(function () {
  ...
  // Ensure Tailwind is present for utility classes
  ...
  st.textContent =
    '#grok-widget{all:initial;font-family:system-ui,sans-serif;position:fixed;bottom:24px;right:24px;z-index:9999;' +
    'width:384px;height:560px;background:#0f172a;border:1px solid rgba(167,139,250,.3);border-radius:24px;' +
    'overflow:hidden;box-shadow:0 25px 60px rgba(0,0,0,.6);display:flex;flex-direction:column}' +
  ...
  widget.innerHTML =
    '<div style="background:linear-gradient(90deg,#6d28d9,#9333ea);padding:16px 20px;display:flex;align-items:center;gap:12px;color:#fff;flex-shrink:0;position:relative">' +
      '<span style="font-size:22px;font-weight:700">𝕏</span>' +
      '<div style="font-weight:600;font-size:15px">Grok • CryptoLoveYou AI</div>' +
      '<div style="position:absolute;right:20px;top:14px;cursor:pointer;font-size:20px;opacity:.8;line-height:1" id="grok-close">✕</div>' +
      '<div style="position:absolute;right:48px;top:14px;font-size:20px">❤️</div>' +
    '</div>' +
    ...
    '<div style="padding:14px 16px;border-top:1px solid rgba(255,255,255,.08);display:flex;align-items:center;gap:10px;background:#0f172a;flex-shrink:0">' +
      '<button id="grok-mic" style="width:44px;height:44px;border-radius:14px;background:rgba(255,255,255,.1);border:none;cursor:pointer;font-size:22px;display:flex;align-items:center;justify-content:center;flex-shrink:0">🎤</button>' +
      '<input id="grok-input" type="text" placeholder="Ask Grok anything..." ' +
        'style="flex:1;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);border-radius:999px;padding:10px 18px;color:#fff;font-size:13px;outline:none">' +
      '<button id="grok-send" style="padding:10px 22px;background:linear-gradient(90deg,#7c3aed,#9333ea);color:#fff;border:none;border-radius:999px;font-size:13px;font-weight:500;cursor:pointer;flex-shrink:0">Send</button>' +
    '</div>'
  ...
  // Welcome message
  addBotMsg('Hey! I\'m Grok from CryptoLoveYou ❤️ You can ask me <strong style="color:#c4b5fd">anything</strong> — crypto, AI, life, markets, or just chat.')
  ...
  function addBotMsg (html)  { addBubble(html, false) }
  ...
  async function sendMessage () {
    ...
    var thinking = addBubble('<span style="opacity:.6">💭 Thinking…</span>', false)
    try {
    ...
    try {
      ...
      grokRecorder.onstop = async function () {
        ...
        try {
          ...
          else addBotMsg('⚠️ Could not transcribe audio. Please type your message.')
        } catch (err) {
